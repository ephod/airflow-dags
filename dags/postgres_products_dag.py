...
@dag(default_args=default_args, tags=['postgres', 'products', 'minio'])
def postgres_products_pipeline():
    @task()
    def start_db():
    ...
    def insert_into_table():
        CSV_FILE = Path('raw/products/products_rejoined_v2.csv')
        download_and_insert_into_table(CSV_FILE)

    start_db = start_db()
    CREATE_TABLE = Path('./dags/sql/create_table_products.sql')
    ...
    start_db >> create_products_table >> insert_into_table >> end_db


postgres_products_dag = postgres_products_pipeline()
